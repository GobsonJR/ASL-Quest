"""Phase 7 Task 8: representation analysis for the frozen I3D features, repeating the
Phase 5 methodology (class silhouette, signer silhouette, PCA) for direct comparison
against the ResNet18 frozen/layer4/appearance-aug numbers already on record.

Operates directly on the cached test-split I3D features (no I3D forward pass here —
the features were already extracted, frozen, by
scripts/extract_i3d_features_native_10.py)."""
...
from scripts.train_native_10_i3d_frozen import load_split

OUT = ROOT / "outputs" / "asl_citizen_native_10" / "experiments" / "i3d_frozen" / "diagnostics"

PHASE_4B_CLASS_SIL = -0.0725
PHASE_4B_SIGNER_SIL = 0.205
PHASE_4C_CLASS_SIL = -0.0769
PHASE_4C_SIGNER_SIL = 0.094
PHASE_6_CLASS_SIL = -0.0723
PHASE_6_SIGNER_SIL = 0.2167
...
def main() -> None:
    ...
    comparison = {
        ...
        "class_silhouette_improved_vs_4b": (sil_class is not None) and (sil_class > PHASE_4B_CLASS_SIL),
        "signer_silhouette_decreased_vs_4b": (sil_signer is not None) and (sil_signer < PHASE_4B_SIGNER_SIL),
    }
